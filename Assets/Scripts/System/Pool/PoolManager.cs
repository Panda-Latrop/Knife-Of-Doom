...
public class PoolManager : MonoBehaviour
{
    ...
    {
        Pool pool;            
        if (pools.TryGetValue(poolObject.PoolTag, out pool))
        {
            //poolObject.SetPosition(Vector3.zero);
            poolObject.SetRotation(Quaternion.identity);
            poolObject.SetParent(pool.Root);
    ...
    public IPoolObject Pop(IPoolObject poolObject, out PoolPopInfo info, bool addPoolOnFailure = true)
    {
        ...
        {
            IPoolObject result = pool.Pop();
            info = PoolPopInfo.done;
            if (result == null)
            {
                result = ForcePop(pool.Prefab, pool.Root);
